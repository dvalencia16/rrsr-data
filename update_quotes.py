...
def rsi_from_closes(cl, n=14):
    ...
    for i in range(n+1,len(cl)):
        ch=cl[i]-cl[i-1]
        g=max(0,ch); l=max(0,-ch)
        ag=(ag*(n-1)+g)/n
        al=(al*(n-1)+l)/n or 1e-9
    rs=ag/al
...
def fh_closes(sym, cnt=120):
    r=requests.get("https://finnhub.io/api/v1/stock/candle",
                   params={"symbol":sym,"resolution":"D","count":cnt,"token":API}, timeout=10)
    r.raise_for_status()
    j=r.json()
    return j["c"] if j.get("s")=="ok" else []
